...
export class TestService {
  ...
  getQuizByCode(code: string) {
    return this.http.get<Quiz[]>('http://localhost:3000/quizzes?code=' + code);
  }
  ...
  getQuizById(id: number) {
    return this.http.get<Quiz>('http://localhost:3000/quizzes/' + id);
  }

  updateQuizResult(id: number, result: QuizResult) {
    return this.http.put<any>(
      'http://localhost:3000/quizResults/' + id,
      result
    );
  }
